...
recompute_local_uniform_vc(StableVC, State=#state{local_replica=ReplicaId, partition=Partition}) ->
    lists:foreach(
        fun({Replica, Time}) ->
            if
                Replica =/= ReplicaId ->
                    ok = grb_oplog_vnode:check_visible(Partition, Replica, Time);
                true ->
                    ok
            end
        end,
        grb_vclock:to_list(StableVC)
    ),
    State.
...
recompute_local_uniform_vc(StableVC, S=#state{local_replica=LocalId,
    ...
    lists:foreach(
        fun({Replica, Time}) ->
            if
                Replica =/= LocalId ->
                    grb_oplog_vnode:check_visible(Partition, Replica, Time);
                true ->
                    ok
            end
        end,
        grb_vclock:to_list(UniformVC)
    ),
